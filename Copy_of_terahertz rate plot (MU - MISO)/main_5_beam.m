...
grid on;
for k = 1:para.K
    plot(ang_deg, pattern_ang_db.BD(:,k),'LineWidth',1);
end

% ANGLE PLOT (BD REFERENCE) 
k = randi(K);
...
title(sprintf('Angle Beam Pattern Comparison – User %d',k));

legend('BD (reference)','SLNR','MRT','MMSE','Location','best');
xlim([-90 90]);

%% ================== DISTANCE GRID ==================
dist = linspace(0.01,60,700);
pattern_dist = struct();
...
grid on;
for k = 1:para.K
    plot(dist, pattern_dist_db.BD(:,k),'LineWidth',1);
end
...
xlabel('Distance (m)');
ylabel('Normalized Gain (dB)');
title(sprintf('Distance Beam Pattern Comparison – User %d',k));
